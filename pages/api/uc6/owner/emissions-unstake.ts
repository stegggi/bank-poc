...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const rl = bestEffortRateLimit({
    namespace: "uc6:owner:emissions-unstake",
    ip,
    limit: 5,
    windowMs: 60_000,
  });
  ...
  try {
    const body = (req.body || {}) as { message?: string; signature?: string; payload?: unknown };
    ...
    if (!upstream.ok) {
      return res.status(upstream.status).json({
        error: "UC6 bot rejected emissions unstake",
        details: json,
      });
    }
