...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const body = (req.body || {}) as OwnerSettingsRequest;
    const message = String(body.message || "");
    const signature = String(body.signature || "");
    const payload = normalizeOwnerSettings(body.payload || {});
    ...
    if (!challenge) {
      return res.status(403).json({ error: "Challenge missing or expired" });
    }
    if (challenge.usedAtMs) {
      return res.status(403).json({ error: "Challenge was already used" });
    }
    ...
    if (!upstream.ok) {
      return res.status(upstream.status).json({
        error: "UC6 bot rejected owner update",
        details: json,
      });
    }
