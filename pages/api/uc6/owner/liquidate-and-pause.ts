...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const rl = bestEffortRateLimit({
    namespace: "uc6:owner:liquidate-and-pause",
    ip,
    limit: 5,
    windowMs: 60_000,
  });
  ...
  try {
    const body = (req.body || {}) as LiquidateAndPauseRequest;
    ...
    const upstream = await fetchWithTimeout(
      `${base}/owner/liquidate-and-pause`,
      {
        method: "POST",
        headers: {
    ...
    if (!upstream.ok) {
      return res.status(upstream.status).json({
        error: "UC6 bot rejected liquidate-and-pause",
        details: json,
      });
    }
